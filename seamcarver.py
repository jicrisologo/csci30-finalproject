...
class SeamCarver(Picture):
    ...
    def find_vertical_seam(self) -> list[int]:
        ...
        for potential_seam in seam_indices:
            for j in range(self.height() - 1, 0,
                           -1):  # work back up the array along the minimum values, starts at the bottom row and ends at row 1 (second from top)
                ...
                if current == 0:  # left edge
                    ...
                    if M[j - 1][current] <= M[j - 1][current - 1] and M[j - 1][current] <= M[j - 1][
                        current + 1]:  # if c <= l and c <= r
                        potential_seam.append(
                            current)  # then have the next index up be c since it's the minimum out of the 3
                    elif M[j - 1][current - 1] <= M[j - 1][current] and M[j - 1][current - 1] <= M[j - 1][
                        current + 1]:  # if 1 <= c and l <= r
                        potential_seam.append(
                            current - 1)  # then have the next index up be l since it's the minimum out of the 3
                    elif M[j - 1][current + 1] <= M[j - 1][current] and M[j - 1][current + 1] <= M[j - 1][
                        current - 1]:  # if r <= c and r <= l
                        potential_seam.append(
                            current + 1)  # then have the next index up be l since it's the minimum out of the 3
